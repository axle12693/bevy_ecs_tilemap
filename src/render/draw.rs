...
use bevy::{
    ...
    ecs::system::{ SystemParamItem, lifetimeless::{ Read, SQuery, SRes } },
    math::UVec4,
    render::{
...
impl<const I: usize> RenderCommand<Transparent2d> for SetTransformBindGroup<I> {
    ...
    fn render<'w>(
        ...
        uniform_indices: Option<
            (&'w DynamicUniformIndex<MeshUniform>, &'w DynamicUniformIndex<TilemapUniformData>)
        >,
...
for SetMaterialBindGroup<M, I> {
    type Param = (SRes<RenderMaterialsTilemap<M>>, SQuery<&'static MaterialTilemapHandle<M>>);
    type ViewQuery = ();
    ...
    fn render<'w>(
        ...
        if let Ok(material_handle) = material_handles.get(id.0) {
            let bind_group = material_bind_groups.into_inner().get(&material_handle.id()).unwrap();
...
impl RenderCommand<Transparent2d> for DrawMesh {
    ...
    fn render<'w>(
        ...
        if
            let Some(chunk) = chunk_storage
                .into_inner()
                .get(&UVec4::new(chunk_id.0.x, chunk_id.0.y, chunk_id.0.z, tilemap_id.0.index()))
        {
            if
